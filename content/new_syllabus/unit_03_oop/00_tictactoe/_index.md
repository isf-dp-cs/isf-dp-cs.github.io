...
| **Class** | An integer that represents the position in a data structure |
| **Constructor** | A method that creates a new object of a class with default values for the properties/attributes |
| **Method** | A data structure that has a fixed size and will not grow or shrink. |
| **Property/attribute** | A data structure that can change its size as needed as elements are added or deleted. |
